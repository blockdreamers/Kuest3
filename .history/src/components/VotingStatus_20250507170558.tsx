          ...
          <div key={rowIndex} className="flex justify-center gap-6 flex-wrap">
            {row.map((index) => {
              ...
              return (
                <div
                  key={coin.project_id}
                  onClick={() => navigate(`/coin/${coin.project_id}`)}
                  className="relative voting-card pyramid-card group hover:bg-[#DFFF67] hover:text-black transition-all duration-300 transform hover:scale-105 cursor-pointer"
                >
                  {index < 3 && (
                    <img
