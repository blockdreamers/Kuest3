          ...
          <div key={rowIndex} className="flex justify-center gap-6">
            {row.map((index) => {
              ...
              return (
                <div
                  key={coin.project_id}
                  className="relative voting-card pyramid-card transform hover:scale-105 transition-transform duration-200"
                >
                  {/* 🥇 1~3위 뱃지 */}
                  {index < 3 && (
                    <img
                      ...
                      alt={`${index + 1}위 뱃지`}
                      className="absolute top-[-10px] left-[-10px] w-8 h-8 z-10"
                    />
                  )}

                  <div className="flex items-center">
                    <img src={coin.logo} alt={coin.name_ko} className="w-14 h-14 rounded-full mr-4" />
                    <div className="flex flex-col justify-center">
                      <div className={`text-base font-bold ${index < 3 ? 'text-white text-xl' : 'text-white text-sm'}`}>
                        {index + 1}위
                      </div>
                      <div className="text-white font-semibold text-lg">{coin.name_ko}</div>
                      <div className="text-xs text-gray-400">{coin.symbol}</div>
                    </div>
                  </div>

                  <div className="absolute bottom-3 right-4 text-orange-400 font-bold text-sm">
