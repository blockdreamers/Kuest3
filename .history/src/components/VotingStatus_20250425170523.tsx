...
const VotingStatus = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      console.log('✅ Supabase voting_status 응답:', data);
    ...
    <div className="mb-16 px-2">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 md:p-6">
        <div className="space-y-3 max-h-[600px] overflow-y-auto">
          {coins.length === 0 ? (
            <p className="text-gray-400 text-center py-6">표시할 데이터가 없습니다.</p>
          ) : (
            coins.map((coin, index) => (
              <VotingCard key={coin.project_id ?? index} coin={coin} rank={index + 1} />
            ))
          )}
        </div>
      </div>
