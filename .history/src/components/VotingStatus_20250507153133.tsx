  ...
  return (
    <div className="mb-16 px-2">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-white">🗳️ 인기 투표</h1>
        <select
          ...
          <div key={rowIndex} className="flex justify-center gap-6 flex-wrap">
            {row.map((index) => {
              ...
              return (
                <div
                  key={coin.project_id}
                  className="relative voting-card pyramid-card group transition-all duration-300 transform hover:scale-105"
                >
                  {index < 3 && (
                    <img
                      src={`/assets/${ordinal}.png`}
                      alt={`${index + 1}위 뱃지`}
                      className="absolute -top-3 -left-3 w-7 h-7 z-10"
                    />
                  )}
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="flex flex-col justify-center">
                      <div className={`font-bold ${index < 3 ? 'text-base' : 'text-sm'}`}>
                        {index < 3 ? `🔥 ${index + 1}위` : `${index + 1}위`}
                      </div>
                      <div className="voting-name text-md">{coin.name_ko}</div>
                      <div className="voting-symbol text-xs text-gray-400 group-hover:text-gray-400">
                        {coin.symbol}
                      </div>
                    </div>
                  </div>
                  ...
                  <button className="absolute bottom-[-40px] left-1/2 transform -translate-x-1/2 group-hover:bottom-2 bg-black text-white px-3 py-1 rounded-md text-xs font-semibold transition-all duration-300">
                    투표하러 가기
                  </button>
                </div>
              );
