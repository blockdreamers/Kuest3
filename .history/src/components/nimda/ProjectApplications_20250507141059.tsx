...
const ProjectApplications = () => {
  ...
  const [modalState, setModalState] = useState<{
    visible: boolean;
    message: string;
    options: { label: string; color: string; action: () => void }[];
  }>({ visible: false, message: "", options: [] });
  ...
  const fetchApplications = async () => {
    try {
      ...
      if (!Array.isArray(json)) throw new Error("형식 오류");
      ...
      return (
        <button
          ...
          onClick={() => {
            setModalState({
              visible: true,
              message: "거절된 신청입니다. 처리 선택:",
              options: [
                {
                  label: "보류처리",
                  color: "bg-yellow-400",
                  action: () => updateStatus(app.id, "pending"),
                },
                {
                  label: "승인처리",
                  color: "bg-green-500",
                  action: () => handleApprove(app),
                },
              ],
            });
          }}
    ...
    if (app.status === "pending") {
      return (
        <div className="flex flex-col gap-1">
          ...
          <button className="bg-red-400 text-black px-2 py-1 rounded" onClick={() => updateStatus(app.id, "rejected")}>거절</button>
            ...
            setModalState({
              ...
              options: [
                ...
                },
                {
                  label: "거절처리",
                  color: "bg-red-500",
                  action: () => updateStatus(app.id, "rejected"),
                },
              ],
            });
          }}
      ...
      {modalState.visible && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-white text-black p-6 rounded shadow-md">
            <p className="mb-4 font-semibold">{modalState.message}</p>
            <div className="flex gap-3">
              {modalState.options.map((opt, i) => (
                <button
                  key={i}
                  className={`${opt.color} text-black font-semibold px-4 py-2 rounded`}
                  onClick={() => {
                    opt.action();
