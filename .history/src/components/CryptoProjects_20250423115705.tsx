...
const CryptoProjects = () => {
  ...
  return (
    <div className="mb-12 font-['Montserrat','Pretendard']">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-6 items-start">
        {displayCoins.map(({ id, name, symbol, logo, price, priceChange, marketCap, volume }) => (
          <Link
            key={id}
            to={`/coin/${id}`}
              ...
              <div className="space-y-2 text-white">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-white">
                  <div>
                    ...
                    <p className="font-medium">${(marketCap / 1e9).toFixed(1)}B</p>
                  </div>
                  <div>
                    <p className="text-gray-400">거래량 (24h)</p>
                    <p className="font-medium">${(volume / 1e9).toFixed(1)}B</p>
                  </div>
                </div>
              </div>
            </div>
