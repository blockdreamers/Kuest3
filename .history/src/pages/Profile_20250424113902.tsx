...
function Profile() {
  ...
  useEffect(() => {
    console.log('📦 privyUser.wallet:', privyUser?.wallet);
    if (privyUser?.wallet?.walletAddress) {
      console.log('✅ [Profile] 지갑 주소 확보됨:', privyUser.wallet.walletAddress);
      setWalletAddress(privyUser.wallet.walletAddress);
    } else {
      console.warn('❌ [Profile] privyUser.wallet.walletAddress 없음');
    }
  }, [privyUser?.wallet]);
  ...
  useEffect(() => {
    const tryInsertWallet = async () => {
      if (firebaseUser && walletAddress && !walletInserted) {
        try {
          console.log('📨 Supabase insert 시도:', walletAddress);
        ...
        console.log('⏭️ Supabase insert 조건 미충족:', {
          firebaseUser,
          walletAddress,
          walletInserted,
        });
