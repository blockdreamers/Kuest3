...
// ✅ 이 주소가 Remix에서 배포한 AirdropClaim 컨트랙트 주소와 같아야 함
...
const Airdrop = () => {
  ...
  const getProvider = (): ethers.providers.Provider | null => {
    ...
    if (wallet?.getEthersProvider) {
      ...
      return wallet.getEthersProvider();
    }
  ...
  const fetchAirdropInfo = async () => {
    try {
      if (!authenticated || !user?.wallet?.address) {
        console.warn('❌ User not authenticated or wallet not available');
        return;
      }
      ...
      alert('에어드랍 정보를 불러오는데 실패했습니다. 콘솔 로그를 확인해주세요.');
    }
  };
