...
function Profile() {
  const { user: privyUser, ready: privyReady, login: privyLogin, logout: privyLogout } = usePrivy();
  ...
  // 🔁 지갑 강제 재연결 함수 (세션 초기화 → 로그인)
  const handleReconnect = async () => {
    try {
      console.log('🔄 [Privy] 로그아웃 후 재연결 시도');
      await privyLogout();
      await privyLogin(); // 또는 privyLogin({ forceLogin: true });
    } catch (err) {
      console.error('❌ [Privy] 재연결 실패:', err);
    }
  };
