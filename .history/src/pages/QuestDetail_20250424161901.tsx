        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {mockParticipants.map((p, i) => (
              <tr key={i}>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{p.walletAddress}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{p.reward} USDT</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{p.participatedAt}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
const QuestDetail = () => {
  ...
  const questData = {
    ...
    claimedReward: 6500,
    tasks: [
      { id: "twitter-follow", platform: "twitter", title: "Follow on Twitter", description: "Follow @CryptoProject on Twitter", link: "https://twitter.com/CryptoProject" },
      { id: "twitter-retweet", platform: "twitter", title: "Retweet Announcement", description: "Retweet our latest announcement", link: "https://twitter.com/CryptoProject/status/123456789" },
      { id: "telegram-join", platform: "telegram", title: "Join Telegram Group", description: "Join our official Telegram group", link: "https://t.me/CryptoProject" },
      { id: "discord-join", platform: "discord", title: "Join Discord Server", description: "Join our Discord community", link: "https://discord.gg/cryptoproject" }
    ],
    questGuide: [
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-['Montserrat','Pretendard']">
      <button
        onClick={() => navigate(-1)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xl font-bold text-green-500">{questData.reward} {questData.rewardType}</span>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {questData.tasks.map((task) => (
                <div
                  key={task.id}
                  className={`${styles.questCard} ${completedTasks.includes(task.id) ? 'bg-green-50 border-green-200' : ''}`}
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div>
                      {completedTasks.includes(task.id) ? (
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${styles.completedBadge}`}>
                        ...
                        <a
                          ...
                          className={`inline-flex items-center px-4 py-2 text-sm font-medium rounded-md ${styles.taskButton}`}
                        >
                          미션 완료하기
            ...
            <div className="mt-8 border-t pt-6">
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                <ul className="space-y-2">
                  {questData.questGuide.map((guide, i) => (
                    <li key={i} className="text-gray-700 flex items-start"><span className="mr-2">•</span>{guide}</li>
                  ))}
                </ul>
              </div>
            </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-lg p-6 sticky top-24">
            <h2 className="text-lg font-semibold mb-4">퀘스트 현황</h2>
            <div className="mb-6">
            ...
            <div className="space-y-4 text-sm">
              <div className="flex justify-between text-gray-600">
                <div className="flex items-center"><Users className="h-4 w-4 mr-2" />참여자 수</div>
                <span className="font-medium">{questData.participants.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-gray-600">
                <div className="flex items-center"><Timer className="h-4 w-4 mr-2" />마감일</div>
            ...
            >
              {completedTasks.length === questData.tasks.length ? '보상 받기' : `미션 완료 (${completedTasks.length}/${questData.tasks.length})`}
            </button>
