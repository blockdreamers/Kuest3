...
const Airdrop = () => {
  ...
  // Privy 연결된 wallet 주소 가져오기
  useEffect(() => {
    if (privyReady) {
      console.log('🧩 Privy Ready 상태 확인됨');
      if (privyUser?.wallet?.address) {
        console.log('✅ Privy 유저 지갑 주소:', privyUser.wallet.address);
        setWalletAddress(privyUser.wallet.address);
      } else {
        console.warn('❌ Privy 지갑 주소 없음, 연결 필요');
      }
    }
  ...
  const fetchAirdropInfo = async (address: string) => {
    try {
      ...
      console.log('📦 Airdrop 수량:', amountFormatted);
      console.log('🪙 이미 클레임했는가?:', claimed);
    } catch (error) {
        ...
        <div className="bg-white rounded-lg border border-gray-300 shadow p-6 mt-2 space-y-4 text-black">
          ...
          {walletAddress ? (
            ...
            <button
              ...
              className="w-full py-3 mt-4 rounded-lg bg-[#C7EB3E] text-black font-semibold text-lg hover:bg-[#b9dc38] transition"
            >
              🔌 지갑 연결하기
            </button>
          )}
