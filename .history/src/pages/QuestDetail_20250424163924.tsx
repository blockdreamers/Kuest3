...
const QuestDetail = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-['Pretendard','Montserrat']">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div>
                    {completedTasks.includes(task.id) ? (
                      ...
                      >
                        미션 완료하기 <ExternalLink className="h-4 w-4 ml-1" />
                      </a>
                    )}
                  </div>
                </div>
            ...
            <div className="mt-8 border-t pt-6">
              ...
              <div className={styles.guideBox}>
                <ul className="space-y-2">
                  {questData.questGuide.map((g, idx) => (
                    <li key={idx} className="text-gray-700 flex items-start"><span className="mr-2">•</span>{g}</li>
                  ))}
                </ul>
              </div>
            </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className={styles.sidebarBox}>
            ...
            <div className="text-sm space-y-2">
              <div className="flex justify-between"><span><Users className="inline h-4 w-4 mr-1" />참여자 수</span><span>{questData.participants}</span></div>
              <div className="flex justify-between"><span><Timer className="inline h-4 w-4 mr-1" />마감일</span><span>{new Date(questData.deadline).toLocaleDateString()}</span></div>
            </div>
            <button
          ...
          <div className={styles.sidebarBox}>
            ...
            <div className="overflow-auto rounded-md">
              <table className="min-w-full divide-y divide-gray-200 participantTable">
                <thead>
                  <tr>
                    <th className="px-4 py-2 text-left">주소</th>
                    <th className="px-4 py-2 text-left">보상</th>
                    <th className="px-4 py-2 text-left">참여시각</th>
                  </tr>
                </thead>
                <tbody>
                  {questData.participants.map((p, i) => (
                    <tr key={i}>
                      <td className="px-4 py-2">{p.walletAddress}</td>
                      <td className="px-4 py-2">{p.reward} USDT</td>
                      <td className="px-4 py-2">{p.participatedAt}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
