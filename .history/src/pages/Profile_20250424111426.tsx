...
function Profile() {
  ...
  useEffect(() => {
    const fetchWallet = async () => {
      try {
        if (privyUser) {
          const wallets = await privyUser.getWallets();
          console.log('🧾 getWallets 결과:', wallets);
          const firstWallet = wallets[0];
          if (firstWallet?.address) {
            setWalletAddress(firstWallet.address);
            console.log('✅ 지갑 주소 확보됨:', firstWallet.address);
          } else {
            console.log('❌ 지갑 주소 없음 (getWallets 반환값 없음)');
          }
        } else {
          console.log('❌ Privy user가 없음');
        }
      } catch (error) {
        console.error('❌ getWallets 실행 오류:', error);
      }
    };
    fetchWallet();
  }, [privyUser]);
