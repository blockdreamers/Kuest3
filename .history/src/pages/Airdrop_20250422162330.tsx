...
const AIRDROP_CONTRACT_ADDRESS = '0x83f954578bc8B4732E1957a439dB1Cb60A47Fc5A'; // 실제 컨트랙트 주소
  ...
  const fetchAirdropInfo = async () => {
    try {
      if (!authenticated || !user?.wallet?.address) {
        console.warn('❌ Privy 인증 or wallet 주소 없음');
        return;
      }

      const provider = getProvider();
      if (!provider) {
        alert('🛑 MetaMask를 감지할 수 없습니다.');
        return;
      }
