...
const Airdrop = () => {
  ...
  const getInfuraProvider = (): ethers.providers.JsonRpcProvider => {
    console.log('📡 [Infura] Provider 준비됨:', INFURA_SEPOLIA_URL);
    return new ethers.providers.JsonRpcProvider(INFURA_SEPOLIA_URL);
  };
  ...
  const fetchAirdropInfo = async () => {
    ...
    try {
      if (!walletAddress) {
        console.warn('❌ [fetchAirdropInfo] 지갑 주소 없음');
        return;
      }
      ...
      console.log('📡 [Infura] airdropAmount 요청 중...');
      const rawAmount = await contract.airdropAmount(walletAddress);
      console.log('📨 [컨트랙트 응답] airdropAmount:', rawAmount.toString());

      console.log('📡 [Infura] claimed 요청 중...');
      const claimed = await contract.claimed(walletAddress);
      console.log('📨 [컨트랙트 응답] claimed:', claimed);
